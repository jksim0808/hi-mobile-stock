...
st.title("📊 하이모바일 3단계 주식 스크리닝 매니저")
st.caption("50대 유망 종목 실시간 모멘텀 필터링 시스템")

# 50대 유망 종목 마스터 데이터 고정 (수동 입력 불편 해소)
FIXED_STOCKS = {
    ...
    "HD현대인프라코어": "042670", "한국항공우주": "047810", "한화에어로스페이스": "012450", "LIGnex1": "079550", "두산로보틱스": "454910",
...
# 메인 제어 컨트롤러
# ==========================================
st.markdown("### 🔍 50대 종목 실시간 모멘텀 스크리닝")
st.write("아래 버튼을 누르면 50개 전 종목의 시세를 실시간 분석하여 현재 매수 적합도를 3단계로 일괄 분류합니다.")

if st.button("🚀 50대 기업 전수 분석 시작", use_container_width=True):
    ...
    total_stocks = len(FIXED_STOCKS)
    
    # 50개 종목 순회 분석
    for idx, (name, code) in enumerate(FIXED_STOCKS.items()):
        status_text.text(f"⏳ 데이터 수집 및 연산 중 ({idx+1}/{total_stocks}): {name} ({code})")
        progress_bar.progress((idx + 1) / total_stocks)
        
        df = get_mobile_naver_data(code)
        if df.empty:
            time.sleep(0.1)
            df = get_mobile_naver_data(code) # 실패 시 1회 재시도
    ...
    status_text.text("✅ 50개 종목 전수 스크리닝이 완료되었습니다!")
    progress_bar.empty()
