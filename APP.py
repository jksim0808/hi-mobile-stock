...
# ==========================================
# 볼륨 가중형(Volume-Weighted) 3단계 판단 로직 매매 전략
# ==========================================
with st.expander("📖 볼륨 가중형(Volume-Weighted) 3단계 판단 로직 매매 전략", expanded=False):
    ...
    *   **매매 해석**: 추세는 살아있으나 당장 강하게 뚫고 갈 에너지가 부족(거래량 부족)하거나, 이미 너무 많이 올라 단기 조정 리스크가 높은 자리입니다. 관망하며 거래량이 실리는 양봉이 떴을 때 진입을 조율해야 합니다.
    ...
    if not current_stocks_map:
        ...
        for idx, (name, code) in enumerate(current_stocks_map.items()):
            ...
            if not df.empty:
                ...
                # [⚠️ ZeroDivisionError 방지 안전 보정]
                # 5일 평균 거래량이 0이거나 결측치일 경우 분모가 0이 되는 것을 방지
                if vol_ma5 > 0:
                    vol_ratio = curr_vol / vol_ma5
                else:
                    vol_ratio = 0.0  # 평소 거래량이 없던 종목은 비율을 0으로 처리
