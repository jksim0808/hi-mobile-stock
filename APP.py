...
st.caption("추세 + RSI 보정 + 거래량(Volume) 가중치 통합 스크리닝 시스템")
...
# ==========================================
# 거래량 지표가 포함된 판단 로직 설명서
# ==========================================
with st.expander("📖 볼륨 가중형(Volume-Weighted) 3단계 판단 로직 매매 전략", expanded=False):
    st.markdown("""
    본 엔진은 기존의 '추세+RSI 보정'에 **'최근 5일 평균 거래량 대비 오늘 수급 강도(Volume Ratio)'**를 결합하여 신뢰도를 극대화했습니다.
    
    ### 1️⃣ 📈 매수 긍정 (수급 동반 강세장 눌림목 및 돌파 구간)
    *   **정량적 기준**: `현재가 > 20일선 > 60일선` **AND** `45 ≤ RSI ≤ 65` **AND** `오늘 거래량 ≥ 최근 5일 평균 거래량의 90%`
    *   **매매 해석**: 대세 상승 추세를 유지하는 가운데 단기 과열이 식은 상태(RSI 45~65)에서, **동시에 오늘 거래량이 평소 수준 이상으로 매끄럽게 유입되는 종목**입니다. 가짜 반등을 걸러낸 진짜 돈이 들어오는 맥점입니다.
    
    ### 2️⃣ ⚠️ 진입 조율 필요 (거래량 미달 횡보 또는 단기 과열)
    *   **정량적 기준**: 정배열 추세는 맞으나 `RSI가 과열(>65)`되거나, 혹은 RSI는 적당한데 `오늘 거래량이 평소의 90% 미만`으로 메말라 있는 경우
    *   **매매 해석**: 추세는 살아있으나 당장 강하게 뚫고 갈 에너지가 부족(거래량 부족)하거나, 이미 너무 많이 올라 단기 조정 리스크가 높은 자리입니다. 관망하며 거래량이 실리는 양봉이 뜰 때까지 진입을 조율해야 합니다.
    
    ### 3️⃣ 💤 관망 권장 (역배열 하락세 또는 하방 압력 심화)
    *   **정량적 기준**: 이평선 조건 미달 (`현재가 < 20일선` 또는 `20일선 < 60일선`)
    *   **매매 해석**: 거래량이 터지든 터지지 않든 매물 벽이 두껍고 하락 역배열이 진행 중인 리스크 구간입니다. 섣부른 저가 매수(물타기)를 철저히 제한하고 자금을 지켜야 하는 소외주 분류입니다.
    """)
...
# 메인 제어 분석 실행
# ==========================================
if st.button("🚀 볼륨 가중형 스크리닝 전수 분석 시작", use_container_width=True):
    if not current_stocks_map:
        st.error("분석할 종목이 없습니다. 위의 편집창 확인 요망.")
    else:
        group_success = []  # 📈 매수 긍정
        ...
        for idx, (name, code) in enumerate(current_stocks_map.items()):
            status_text.text(f"⏳ 추세/RSI/거래량 분석 중 ({idx+1}/{total_stocks}): {name} ({code})")
            ...
            if not df.empty:
                ...
                # 2. 최근 5일 평균 거래량(오늘 제외) 계산 및 오늘 거래량 비교
                ...
                vol_ma5 = float(df['Vol_MA5'].iloc[-1]) if not pd.isna(df['Vol_MA5'].iloc[-1]) else 1.0
                
                # 평소 대비 오늘 거래량 비율 (예: 1.2 = 평소의 120%)
                ...
                stock_info = {
                    ...
                    "RSI": f"{curr_rsi:.1f}",
                    "거래량 비율": f"{vol_ratio * 100:.1f}%"
                }
                
                # 3. [복합 판단 조건 분기]
                is_trending = curr_price > ma20 > ma60  # 중장기 정배열 추세
                is_bullish_pullback = 45 <= curr_rsi <= 65  # 단기 과열 해소 안전지대
                is_volume_active = vol_ratio >= 0.9  # 거래량이 평소 수준(90%) 이상으로 유입 중
                
                if is_trending and is_bullish_pullback and is_volume_active:
                    group_success.append(stock_info)
                elif is_trending:
                    # 정배열이지만 거래량이 없거나, RSI가 너무 과열되었거나 낙폭이 과한 경우
        ...
        status_text.text(f"✅ 총 {total_stocks}개 종목의 볼륨 복합 스크리닝이 완료되었습니다!")
        progress_bar.empty()
        
        # ==========================================
        # 3분할 대시보드 결과 출력 (거래량 비율 컬럼 추가 연동)
        # ==========================================
        st.markdown("---")
        ...
        with col1:
            ...
            st.caption("강세 정배열 + RSI 안정권 + 거래량 유입(5일 평균 90% 이상)")
            if group_success:
        ...
        with col2:
            ...
            st.caption("정배열이나 단기 과열 상태 또는 거래량 부재로 숨고르기 중")
            if group_warning:
        ...
        with col3:
            ...
            st.caption("현재 하락 역배열 구간이거나 대세 하방 압력 노출 종목")
            if group_info:
